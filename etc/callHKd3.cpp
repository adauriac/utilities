  ...
  for(i=0;i<V*x;i++)
    {
      ...
      c.z = 0;
      s.push_back(c);
      site[_INDEX_SITE_3(c.x,c.y,c.z)] = 1;
    }
  int *lab = labelise_alahk(s,L1,L2,L3,per);
  labelise_alahk(site,L1,L2,L3,per);
  for(i=0;i<V;i++)
    if (site[i]!=lab[i])
      _STOP("i= %d lab[i]= %d site[i]= %d\n",i,lab[i],site[i]);
  printf("Les deux determinations sont les meme: YOUPEE\n");
